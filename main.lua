...
-- Axiom Build | Rayfield v1 | Cleaned & Fixed
-- ============================================================
...
-- TYCOON
local Tycoons = Workspace:FindFirstChild("Tycoons") or Workspace:WaitForChild("Tycoons", 3)
...
local function copyBase(t)
    local objs = getObjects(t)
    if not objs then return nil, 0 end
    ...
    for _, obj in ipairs(objs:GetDescendants()) do
        if obj:IsA("Model") or obj:IsA("BasePart") then
            ...
            if obj:IsA("Model") and obj.PrimaryPart then
                rel = bCF:ToObjectSpace(obj.PrimaryPart.CFrame)
            elseif obj:IsA("BasePart") then
                rel = bCF:ToObjectSpace(obj.CFrame)
            end
...
local function pasteBase(list)
    ...
    for _, d in ipairs(list) do
        table.insert(payload, {name = d.name, cframe = d.relativeCFrame})
    end
...
local function applySpeed(on, mult)
    ...
    State.conns.speed = RunService.Heartbeat:Connect(function()
        ...
        if c then
            local h = c:FindFirstChildOfClass("Humanoid")
            if h then h.WalkSpeed = 16 * mult end
        end
    end)
end
...
local function applyFullbright(on)
    if on then
        origL.B = Lighting.Brightness
        origL.G = Lighting.GlobalShadows
        origL.A = Lighting.Ambient
        ...
        Lighting.Brightness = origL.B or 1
        Lighting.GlobalShadows = origL.G ~= nil and origL.G or true
...
local function teleport(t)
    ...
    local primary = t:FindFirstChild("PrimaryPart")
    if primary then
        root.CFrame = primary.CFrame + Vector3.new(0, 6, 0)
        return
    end
    local maps = t:FindFirstChild("Maps")
    if maps then
        local ext = maps:FindFirstChild("Exterior")
        if ext then root.CFrame = ext.CFrame + Vector3.new(0, 6, 0) end
    end
end
...
BaseTab:CreateDropdown({
    Name = "Kopyalanacak Tycoon",
    Options = CLASSES,
    CurrentOption = CLASSES[1],
    Flag = "selBase",
    Callback = function(v) State.selBase = v end,
})
BaseTab:CreateButton({
    Name = "Analiz Et & Kopyala",
    Callback = function()
        ...
        State.copiedBase = list
        State.copiedCost = total
        State.copiedFrom = State.selBase
        notify("Kopyalandı!", State.selBase .. " → " .. #list .. " nesne | " .. fmt(total), 6)
    end
})
BaseTab:CreateButton({
    Name = "Kendi Base'ime Yapıştır (Toplu)",
    Callback = function()
...
BaseTab:CreateButton({
    Name = "Kopyalanan Maliyeti Göster",
    Callback = function()
...
BaseTab:CreateButton({
    Name = "Base Wipe (wipeBuilds)",
    Callback = function()
        local ok, err = fire("wipeBuilds")
        notify(ok and "Wipe Edildi" or "Hata", ok and "Base temizlendi." or tostring(err))
    end
})
BaseTab:CreateButton({
    Name = "Tycoon Recover",
    Callback = function()
        local ok, err = fire("recoverTycoon")
        notify(ok and "Recover Başladı" or "Hata", tostring(err))
    end
})
BaseTab:CreateButton({
    Name = "Tycoon Export",
    Callback = function()
        ...
        if not myT then notify("Hata", "Tycoon yok."); return end
        local ok, err = fire("exportTycoon", myT.Name)
        notify(ok and "Export Başladı" or "Hata", tostring(err))
    end
})
BaseTab:CreateButton({
    Name = "Blueprint Kaydet",
    Callback = function()
        if not State.copiedBase then notify("Hata", "Önce kopyala."); return end
...
PlayerTab:CreateToggle({
    Name = "Godmode (Client Health Lock)",
    CurrentValue = false,
    Flag = "godOn",
...
NPCTab:CreateButton({
    Name = "Tüm NPC'leri Kov",
    Callback = function()
        local ok, err = fire("fireAllNPCs")
        notify(ok and "NPC'ler Kovuldu" or "Hata", ok and "Tüm NPC işten çıktı." or tostring(err))
    end
})
NPCTab:CreateButton({
    Name = "Tüm NPC Komutlarını Temizle",
    Callback = function()
        local ok, err = fire("clearAllNPCCommands")
        notify(ok and "Temizlendi" or "Hata", tostring(err))
    end
})
NPCTab:CreateButton({
    Name = "NPC Talep Et",
    Callback = function()
        local r = GE and GE:FindFirstChild("requestNPCs")
        if not r then notify("Hata", "Remote yok."); return end
        local ok, err = pcall(function() r:FireServer() end)
        notify(ok and "NPC Talep Edildi" or "Hata", tostring(err))
    end
})
NPCTab:CreateButton({
    Name = "NPC Debug",
    Callback = function()
        local ok, err = fire("debugNPC")
        notify(ok and "Debug Atıldı" or "Hata", tostring(err))
    end
})
...
TycoonTab:CreateButton({
    Name = "Tüm Tycoon'ları Tara",
    Callback = function()
        local lines = {}
        for _, t in ipairs(getAllTycoons()) do
            local objs = getObjects(t)
            local cnt = objs and #objs:GetChildren() or 0
...
TycoonTab:CreateButton({
    Name = "Tycoon Oluştur (Safe)",
    Callback = function()
        local ok, err = fire("createTycoon", "Safe")
        notify(ok and "Oluşturuldu" or "Hata", ok and "Safe tycoon oluşturuldu." or tostring(err))
    end
})
TycoonTab:CreateButton({
    Name = "Tycoon Yükle",
    Callback = function()
        local ok, err = fire("loadTycoon", State.selTycoon)
        notify(ok and "Yüklendi" or "Hata", ok and State.selTycoon .. " yüklendi." or tostring(err))
    end
})
TycoonTab:CreateSection("Simülasyon")
TycoonTab:CreateToggle({
    Name = "Simülasyonu Durdur",
    CurrentValue = false,
    Flag = "simPause",
    Callback = function(v)
...
TycoonTab:CreateButton({
    Name = "Co-op'tan Ayrıl",
    Callback = function()
        local ok, err = fire("leaveCoopTycoon")
        notify(ok and "Ayrıldı" or "Hata", tostring(err))
    end
})
TycoonTab:CreateButton({
    Name = "Tesis Sorunlarını Göster",
    Callback = function()
        local r = R("getTycoonIssues")
        if not r then notify("Hata", "Remote yok."); return end
        local conn
        conn = r.OnClientEvent:Connect(function(data)
            conn:Disconnect()
            if data then
                local lines = {}
                for k, v in pairs(data) do table.insert(lines, tostring(k) .. ": " .. tostring(v)) end
...
RewardTab:CreateButton({
    Name = "Daily Reward Al",
    Callback = function()
        local ok, err = fire("claimDailyReward")
        notify(ok and "Daily Alındı!" or "Hata", ok and "Claim başarılı." or tostring(err))
    end
})
RewardTab:CreateButton({
    Name = "Video Reward Al",
    Callback = function()
        local ok, err = fire("doVideoReward")
        notify(ok and "Video Reward Alındı!" or "Hata", tostring(err))
    end
})
RewardTab:CreateButton({
    Name = "Group Reward Al",
    Callback = function()
        local ok, err = fire("claimGroupReward")
        notify(ok and "Group Reward Alındı!" or "Hata", tostring(err))
    end
})
RewardTab:CreateSection("Otomatik")
RewardTab:CreateToggle({
    Name = "Auto Daily+Video+Group (60sn)",
    CurrentValue = false,
    Flag = "autoRewardOn",
    Callback = function(v)
...
RewardTab:CreateButton({
    Name = "Mevcut Quest'i Tamamla",
    Callback = function()
        local ok, err = fire("completeQuest")
        notify(ok and "Quest Tamamlandı!" or "Hata", ok and "XP ve para verildi." or tostring(err))
    end
})
RewardTab:CreateButton({
    Name = "Sonraki SCP Quest Başlat",
    Callback = function()
        local ok, err = fire("startPlaceNextScpQuest")
        notify(ok and "Quest Başladı" or "Hata", tostring(err))
    end
})
RewardTab:CreateSection("Leaderboard")
RewardTab:CreateButton({
    Name = "Leaderboard Çek",
    Callback = function()
        local r = R("getLeaderboard")
        if not r then notify("Hata", "Remote yok."); return end
        local conn
        conn = r.OnClientEvent:Connect(function(data)
            conn:Disconnect()
...
AdminTab:CreateButton({
    ...
    Callback = function()
        local ok, err = fire("triggerFacilityLockdown")
        notify(ok and "LOCKDOWN BAŞLADI!" or "Hata", ok and "Tesis kilitlendi." or tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "Alarmları Aç",
    Callback = function()
        local ok, err = fire("setFacilityAlarms", true)
        notify(ok and "Alarmlar Açıldı" or "Hata", tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "Alarmları Kapat",
    Callback = function()
        local ok, err = fire("setFacilityAlarms", false)
        notify(ok and "Alarmlar Kapatıldı" or "Hata", tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "Breach Tetikle",
    Callback = function()
        local ok, err = fire("adminTriggerBreach")
        notify(ok and "BREACH BAŞLADI!" or "Hata", ok and "SCP breach aktif." or tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "Riot Tetikle",
    Callback = function()
        local ok, err = fire("adminTriggerRiot")
        notify(ok and "RIOT BAŞLADI!" or "Hata", ok and "Riot event aktif." or tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "Bomba Aktifleştir",
    Callback = function()
        local ok, err = fire("activateBomb")
        notify(ok and "Bomba Aktif!" or "Hata", tostring(err))
    end
})
AdminTab:CreateSection("RP Admin")
AdminTab:CreateButton({
    Name = "RP Breach Başlat",
    Callback = function()
        local ok, err = fire("rpAdminStartBreach")
        notify(ok and "RP Breach Başladı" or "Hata", tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "RP Breach Durdur",
    Callback = function()
        local ok, err = fire("rpAdminStopBreach")
        notify(ok and "RP Breach Durdu" or "Hata", tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "RP Sunucu Restart",
    Callback = function()
        local ok, err = fire("rpAdminRestart")
        notify(ok and "RP Restart" or "Hata", tostring(err))
    end
})
AdminTab:CreateButton({
    Name = "Admin Para Ekle",
    Callback = function()
...
MiscTab:CreateButton({
    Name = "Tüm Bildirimleri Kapat",
    Callback = function()
        local ok, err = fire("dismissAllNotifications")
        notify(ok and "Bildirimler Kapatıldı" or "Hata", tostring(err))
    end
})
MiscTab:CreateButton({
    Name = "Para Çarpanı Göster",
    Callback = function()
        local r = R("getMoneyMultiplier")
        if not r then notify("Hata", "Remote yok."); return end
        local conn
        conn = r.OnClientEvent:Connect(function(data)
            conn:Disconnect()
            notify("Para Çarpanı", "x" .. tostring(data), 5)
        end)
        r:FireServer()
    end
})
MiscTab:CreateButton({
    Name = "Tutorial Yeniden Başlat",
    Callback = function()
        local ok, err = fire("restartTutorial")
        notify(ok and "Tutorial Başladı" or "Hata", tostring(err))
    end
})
MiscTab:CreateButton({
    Name = "Ayarları Sıfırla",
    Callback = function()
        local ok, err = fire("resetSettings")
        notify(ok and "Ayarlar Sıfırlandı" or "Hata", tostring(err))
    end
})
MiscTab:CreateSection("Proxy View")
MiscTab:CreateButton({
    Name = "Proxy View İste",
    Callback = function()
        local ok, err = fire("requestProxyView", State.selTycoon)
        notify(ok and "Proxy View Aktif" or "Hata", tostring(err))
    end
})
MiscTab:CreateButton({
    Name = "Proxy View'dan Çık",
    Callback = function()
        local ok, err = fire("requestExitProxyView")
        notify(ok and "Çıkıldı" or "Hata", tostring(err))
    end
})
MiscTab:CreateSection("Nesne")
MiscTab:CreateButton({
    Name = "Tüm Nesneleri Sil (deleteObjects)",
    Callback = function()
        local ok, err = fire("deleteObjects")
        notify(ok and "Nesneler Silindi" or "Hata", tostring(err))
    end
})
MiscTab:CreateButton({
    Name = "Nesneleri Gizle/Göster",
    Callback = function()
        local ok, err = fire("setObjectsEnabled", State.selTycoon, false)
        notify(ok and "Nesneler Gizlendi" or "Hata", tostring(err))
    end
})

-- INIT
notify("SCP Architect X | Axiom v4.0", "Yüklendi! Menüyü açıp kapatmak için klavyeden Sağ Shift (RightShift) tuşuna bas.", 6)
